...
What makes it complex: uncommitted changes may conflict, nested directories need recursive handling, and the operation must be atomic to avoid a broken state on crash.

### Q5.2 - Detecting dirty working directory conflicts
For each file in the current index:
1. Run stat() and compare mtime and size with stored index values
2. If they differ, the file is dirty
3. For each dirty file, look up its hash in the target branch tree
4. If the target branch has a different hash, abort checkout with an error

### Q5.3 - Detached HEAD and recovery
Detached HEAD means HEAD contains a raw commit hash. Commits made in this state are not pointed to by any branch. Once you checkout another branch those commits become unreachable.

Recovery: create a branch before switching, or manually create a branch at the lost hash if you remember it.

### Q6.1 - Garbage Collection Algorithm
Mark phase: walk all commits/trees/blobs from every branch ref, add each hash to a reachable set.
Sweep phase: delete any object file not in the reachable set.
Data structure: hash set of 32-byte ObjectIDs for O(1) lookup.
For 100,000 commits and 50 branches: ~1.2 million objects, ~500,000-800,000 unique visits with shared history.

### Q6.2 - GC Race Condition
GC builds reachable set at T1. Commit writes new blobs at T2-T3 but hasn't updated refs yet. GC sweeps at T4 and deletes the new blobs. Commit updates HEAD at T5 — now references deleted objects — repo is corrupt.

Git avoids this with: 2-week grace period for new objects, lock files, bottom-up write order, and marking index entries as reachable.
